...
this.targetSelection=this.dateFocus;this.next();}else{this.next();}}},goToPreviousYear:function(){if(!this.isDateOutOfRangePreviousYear(this.previousYearDate())){var currentElement=this.getCurrentElement();if(currentElement&&this.dateFocus!==null){if(this.minDate!==null&&(this.d.getFullYear())-1===this.minDate.getFullYear()){if(this.d.getMonth()<this.minDate.getMonth()){this.targetSelection=this.minDate;this.deselectCurrentElement();this.d.setFullYear(this.d.getFullYear()-1);this.d.setMonth(this.minDate.getMonth());this.render('left');this.fireEvent('previous');}else if(this.d.getMonth()==this.minDate.getMonth()){if(this.d.getDay()<this.minDate.getDay()){this.targetSelection=this.minDate;this.deselectCurrentElement();this.d.setFullYear(this.d.getFullYear()-1);this.d.setMonth(this.minDate.getMonth());this.render('left');this.fireEvent('previous');}else{this.targetSelection=this.dateFocus.decrement('year',1);this.previousYear();}}else{this.targetSelection=this.dateFocus.decrement('year',1);this.previousYear();}}else{this.targetSelection=this.dateFocus.decrement('year',1);this.previousYear();}}else{this.previousYear();}}},goToNextYear:function(){if(!this.isDateOutOfRangeNextYear(this.nextYearDate())){var currentElement=this.getCurrentElement();if(currentElement&&this.dateFocus!==null){if(this.maxDate!==null&&(this.d.getFullYear())+1===this.maxDate.getFullYear()){if(this.d.getMonth()>this.maxDate.getMonth()){this.targetSelection=this.maxDate;this.deselectCurrentElement();this.d.setFullYear(this.d.getFullYear()+1);this.d.setMonth(this.maxDate.getMonth());this.render('right');this.fireEvent('next');}
else if(this.d.getMonth()==this.maxDate.getMonth()){if(this.d.getDay()>this.maxDate.getDay()){this.targetSelection=this.maxDate;this.deselectCurrentElement();this.d.setFullYear(this.d.getFullYear()+1);this.d.setMonth(this.maxDate.getMonth());this.render('right');this.fireEvent('next');}else{this.targetSelection=this.dateFocus.increment('year',1);this.nextYear()}}else{this.targetSelection=this.dateFocus.increment('year',1);this.nextYear()}}
else{this.targetSelection=this.dateFocus.increment('year',1);this.nextYear();}}else{this.nextYear();}}},getCurrentElement:function(){var currentElement=this.newContents.getElement('.current');if(!currentElement){currentElement=this.dateSelection;}
...
return null;},isDateOutOfRange:function(date){return(this.minDate!==null&&date<this.minDate)||(this.maxDate!==null&&date>this.maxDate);},isDateOutOfRangePreviousYear:function(date){var isDateOutOfRangeMinDate=false;if(this.minDate!==null&&date!==null){if(this.minDate.getFullYear()>date.getFullYear()){isDateOutOfRangeMinDate=true;}}
return isDateOutOfRangeMinDate;},isDateOutOfRangeNextYear:function(date){var isDateOutOfRangeMaxDate=false;if(this.maxDate!==null&&date!==null){if(this.maxDate.getFullYear()<date.getFullYear()){isDateOutOfRangeMaxDate=true;}}
return isDateOutOfRangeMaxDate;},fillCalendarHeader:function(){var month=this.d.getMonth();var dateDuJour=new Date().toLocaleString();if(this.picker.hasClass('dpmetier')){this.picker.getElement('.titre_aujourdhui').set({text:'Aujourd\'hui : '+dateDuJour.substr(0,dateDuJour.length-8),'aria-disabled':this.isDateOutOfRange(this.today)});}else{this.picker.getElement('.titre_aujourdhui').set({title:dateDuJour.substr(0,dateDuJour.length-8),text:'Aujourd\'hui','aria-disabled':this.isDateOutOfRange(this.today)});}
this.picker.getElement('.titre_navigation_mois').set('html',this.options.months[month]);this.picker.getElement('.titre_navigation_mois_prec').set({title:this.options.months[(month+11)%12],'aria-disabled':this.isDateOutOfRange(this.lastDateOfPreviousMonth())});this.picker.getElement('.titre_navigation_mois_suiv').set({title:this.options.months[(month+1)%12],'aria-disabled':this.isDateOutOfRange(this.firstDateOfNextMonth())});this.picker.getElement('.titre_navigation_annee').set('text',this.d.getFullYear());this.picker.getElement('.titre_navigation_annee_prec').set({title:this.d.getFullYear()-1,'aria-disabled':this.isDateOutOfRangePreviousYear(this.previousYearDate())});this.picker.getElement('.titre_navigation_annee_suiv').set({title:this.d.getFullYear()+1,'aria-disabled':this.isDateOutOfRangeNextYear(this.nextYearDate())});},renderMonth:function(){var month=this.d.getMonth();var year=this.d.getFullYear();this.fillCalendarHeader();this.d.setDate(1);while(this.d.getDay()!==this.options.startDay){this.d.setDate(this.d.getDate()-1);}
...
if(available){e.addEvent('click',function(d,e){this.select(d);return false;}.bind(this,{day:this.d.getDate(),month:this.d.getMonth(),year:this.d.getFullYear()}));e.addEvent('mouseover',function(cell){this.deselectCurrentElement();this.selectCurrentElement(cell);}.bind(this,e));}
...
var InputDiacritique=new Class({tabCode:[[101,233,232,234,235],[69,201,200,202,203],[97,224,226,228,230],[65,192,194,196,198],[105,238,239],[73,206,207],[111,244,246,339],[79,212,214,338],[117,249,251,252],[85,217,219,220],[89,376],[121,255],[99,231],[67,199]],charTrigger:'=',initialize:function(element){element.addEvent('keypress',this.addDiacritique.bind(this));element.addEvent('blur',this.removeContent.bind(this));this.elementId='diacritique-text';this.element=$(this.elementId);if(this.element===null){this.element=this.construct(this.elementId);$(document.body).adopt(this.element);}},getSelection:function(elem){var s={start:0,end:0};if(document.activeElement===elem){s.start=elem.selectionStart;s.end=elem.selectionEnd;}
return s;},addDiacritique:function(event){var elem=event.target;var value;var code;var newCode;var s;var tab;var notTrouve=true;if(document.activeElement===elem){s=this.getSelection(elem);if(s.start>0&&event.key===this.charTrigger){value=elem.value;code=value.charCodeAt(s.start-1);newCode=code;for(var i=0;i<this.tabCode.length&&notTrouve;i++){tab=this.tabCode[i];for(var j=0;j<tab.length;j++){if(tab[j]===code){if(j===(tab.length-1)){newCode=tab[0];}else{newCode=tab[j+1];}
notTrouve=false;break;}}}
if(newCode!==code){event.stop();elem.value=value.substr(0,s.start-1)+String.fromCharCode(newCode)+value.substr(s.start,value.length-1);elem.setSelectionRange(s.start,s.end);this.element.set('html',String.fromCharCode(newCode));}}}},construct:function(elementId){var textSpan=new Element('span',{'id':elementId,'class':'hiddenText','aria-live':'polite','aria-atomic':'true'});return textSpan;},removeContent:function(event){this.element.set('html','')}});var ClavierDiacritique=new Class({Implements:[Options],options:{cssclass:null,},attachInput:null,attachButton:null,element:null,elementTitle:null,elementId:null,firstElement:null,lastElement:null,focusElement:null,keyboardHandlerId:null,mouseDownHandlerId:null,getSelection:function(elem){var s={start:0,end:0};if(document.activeElement===elem){s.start=elem.selectionStart;s.end=elem.selectionEnd;}
return s;},initialize:function(input,options){this.setOptions(options);this.elementTitle='Clavier des lettres accentuées';this.elementId='clavier-diacritique';this.element=$(this.elementId);this.attachInput=input;this.buttonTitle="Aide à la saisie des lettres accentuées"
this.attachButton=this.constructButton(input);if(this.element===null){this.element=this.construct(this.elementId);$(document.body).adopt(this.element);}
var focusableElements=this.initFocusableElements(this.element);this.firstElement=focusableElements[0];this.lastElement=focusableElements[focusableElements.length-1];this.focusElement=focusableElements[1];},constructButton:function(input){var button=new Element('button',{'id':input.id+'-button','title':this.buttonTitle,'class':'button-diacritique','type':'button'});button.inject(input,'after');button.addEvent('click',function(event){this.open();}.bind(this));return button;},construct:function(elementId){var fenetre=new Element('div',{'id':elementId,'class':'fenetre','hidden':'hidden','aria-hidden':'true','role':'dialog','tabindex':'0','aria-label':this.elementTitle});if(this.options.cssclass){fenetre.addClass(this.options.cssclass);}
this.constructContent().inject(fenetre);return fenetre;},constructContent:function(){var tabMaj=[{'label':'A grave','text':'&Agrave;'},{'label':'A circonflexe','text':'&Acirc;'},{'label':'A trema','text':'&Auml;'},{'label':"e dans l'a",'text':'&#198;'},{'label':"c cedille",'text':'&Ccedil;'},{'label':"e aiguë",'text':'&Eacute;'},{'label':"e grave",'text':'&Egrave;'},{'label':"e circonflexe",'text':'&Ecirc;'},{'label':"e trema",'text':'&Euml;'},{'label':"i circonflexe",'text':'&Icirc;'},{'label':"i trema",'text':'&Iuml;'},{'label':"O circonflexe",'text':'&Ocirc;'},{'label':"O trema",'text':'&Ouml;'},{'label':"e dans l'o",'text':'&#338;'},{'label':"u grave",'text':'&Ugrave;'},{'label':"u circonflexe",'text':'&Ucirc;'},{'label':"u trema",'text':'&Uuml;'},{'label':"i grec trema",'text':'&Yuml;'}];var tabMin=[{'label':'a grave','text':'&agrave;'},{'label':'a circonflexe','text':'&acirc;'},{'label':'a trema','text':'&auml;'},{'label':"e dans l'a",'text':'&aelig;'},{'label':"c cedille",'text':'&ccedil;'},{'label':"e aiguë",'text':'&eacute;'},{'label':"e grave",'text':'&egrave;'},{'label':"e circonflexe",'text':'&ecirc;'},{'label':"e trema",'text':'&euml;'},{'label':"i circonflexe",'text':'&icirc;'},{'label':"i trema",'text':'&iuml;'},{'label':"O circonflexe",'text':'&ocirc;'},{'label':"O trema",'text':'&ouml;'},{'label':"e dans l'o",'text':'&oelig;'},{'label':"u grave",'text':'&ugrave;'},{'label':"u circonflexe",'text':'&ucirc;'},{'label':"u trema",'text':'&uuml;'},{'label':"i grec trema",'text':'&yuml;'}];var content=new Element('div.clavier');var majuscule=new Element('ul#diacritique-maj[aria-label=majuscule]');var minuscule=new Element('ul#diacritique-min[aria-label=minuscule]');var clicked=false;tabMaj.forEach(function(elem){var li=new Element('li');var button=new Element('button',{'type':'button','aria-label':elem.label,'html':elem.text});button.inject(li);li.inject(majuscule);});tabMin.forEach(function(elem){var li=new Element('li');var button=new Element('button',{'type':'button','aria-label':elem.label,'html':elem.text});button.inject(li);li.inject(minuscule);});majuscule.inject(content);minuscule.inject(content);content.addEvent('click:relay(button)',function(event,target){var elem=this.element.retrieve('attachInput');if(elem!==null){elem.focus();var s=this.getSelection(elem);var value=elem.value;elem.value=value.substr(0,s.start)+target.get('html')+value.substr(s.start,value.length);elem.setSelectionRange(s.start+1,s.start+1);}
this.close();clicked=true;}.bind(this));content.addEvent('focus:relay(button)',function(cell,target){clicked=false;target.addClass('currentDiacritique');});content.addEvent('blur:relay(button)',function(cell,target){target.removeClass('currentDiacritique');});content.addEvent('mouseover:relay(button)',function(cell,target){target.addClass('currentDiacritique');target.focus();});content.addEvent('mouseout:relay(button)',function(cell,target){target.removeClass('currentDiacritique');target.focus();});content.addEvent('mouseout',function(cell,target){if(target===undefined&&cell.target.nodeName==="BUTTON"&&!clicked){cell.target.addClass('currentDiacritique');}});return content;},open:function(){if(this.attachInput.disabled){return;}
this.element.store('attachInput',this.attachInput);this.position(this.attachButton);this.element.removeProperties('hidden','aria-hidden');this.focusElement.focus();$(document).addEvent('mousedown',this.mouseDownHandlerId=this.mouseDownHandler.bind(this));$(document).addEvent('keydown',this.keyboardHandlerId=this.keyboardHandler.bind(this));},close:function(){$(document).removeEvent('mousedown',this.mouseDownHandlerId);$(document).removeEvent('keydown',this.keyboardHandlerId);var elem=this.element.retrieve('attachInput');if(elem!==null){elem.focus();}
this.element.setProperties({'hidden':'hidden','aria-hidden':'true'});},mouseDownHandler:function(event){if(!this.element.contains(event.target)){this.close();}},position:function(attach){var x=attach.getPosition().x+attach.getComputedSize().totalWidth;var y=attach.getPosition().y+attach.getComputedSize().totalHeight;if(this.element.getComputedSize().totalWidth+x<$(document.body).getSize().x){if(this.element.getComputedSize().totalHeight+y<$(document.body).getSize().y){this.element.position({relativeTo:attach,position:'bottomLeft',edge:'upperLeft'});}else{this.element.position({relativeTo:attach,position:'upperLeft',edge:'bottomLeft'});}}else{if(this.element.getComputedSize().totalHeight+y<$(document.body).getSize().y){this.element.position({relativeTo:attach,position:'bottomRight',edge:'upperRight'});}else{this.element.position({relativeTo:attach,position:'upperRight',edge:'bottomRight'});}}},keyboardHandler:function(event){if(this.element.contains(event.target)){if(event.key==='esc'){event.stop();this.close();}else if(event.key==='tab'){if(event.shift){if(this.firstElement===document.activeElement){event.stop();this.lastElement.focus();}}else{if(this.lastElement===document.activeElement){event.stop();this.firstElement.focus();}}}}},initFocusableElements:function(container){var focusableElementsSelector='[tabindex]:not([tabindex^="-"]),a[href]:not([tabindex]),'+'input:not([tabindex]):not([disabled="disabled"]),'+'button:not([tabindex]):not([disabled="disabled"]),'+'select:not([tabindex]):not([disabled="disabled"]),'+'textarea:not([tabindex]):not([disabled="disabled"])';var visibleFocusableElements=container.getElements(focusableElementsSelector).filter(function(item,index){return item.isDisplayed();});return $$(container).concat(visibleFocusableElements);},});
